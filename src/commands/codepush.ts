...
`,
  ...
  async run() {
    ...
    const project =
      target === 'ios'
        ? process.env.CODE_PUSH_IOS_PROJECT
        : process.env.CODE_PUSH_ANDROID_PROJECT;
    shell.exec(
      `appcenter codepush release-react -a ${project} -d ${
        env === 'staging' ? 'Staging' : 'Production'
      } --target-binary-version ${appVersion.version} --token ${
        process.env.CODE_PUSH_TOKEN
      }`,
    );
  }
}
